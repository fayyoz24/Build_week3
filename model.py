...
class Emotion(nn.Module):
    neetwork_config = [32,32,"M", 64,64,"M", 128,128,"M"]

    def __init__(self, num_of_channels, num_of_classes):
        super(Emotion, self).__init__()
        self.features = self._make_layers(num_of_channels, self.neetwork_config)
        self.classifier = nn.Sequential(nn.Linear(6*6*128, 64),
                                        nn.ELU(True),
                                        nn.Dropout(p=0.5),
                                        nn.Linear(64,num_of_classes))

# instructions pytorch how to execute the defined layers in the network
# x --> image
    def forward(self, x):
        out = self.features(x)
